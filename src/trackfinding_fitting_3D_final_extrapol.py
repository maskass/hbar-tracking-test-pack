...
def plot_event_3_views(cuspnr, eventnr, fig, ax, h, list_i, list_o, pmapcI, pmapcO, zposII, zposOO, zerrI, zerrO, width_list_x, width_list_xo, width_list_y, width_list_yo,
    ...
    fig.savefig("./event_plots_3D/event3D_{0}_{1}.pdf".format(cuspnr, eventnr), bbox_inches='tight')      
    #plt.show()
...
def track_finding_4layers(polygon_hitlist, h, nO, nI, I, O, zposII, zposOO,
    ...
    if len(barsi_left) != 0:   # if there are bars with a hit but no matching fibre hit
        ...
        for barsi, xy_, z_, widx_, widy_ in zip(barsi_left, xy, z, widx, widy):
            bi_p_3D.append([z_, zerrI, xy_[0], widx_*0.5, xy_[1], widy_*0.5, barsi, 999, 0, 0])
    ...
    if len(barso_left) != 0:
        ...
        for barso, xy_, z_, widx_, widy_ in zip(barso_left, xy, z, widx, widy):
            bo_p_3D.append([z_, zerrO, xy_[0], widx_*0.5, xy_[1], widy_*0.5, barso, 999, 0, 1])
    ...
    for iz, fi in zip(if_z, curr_fI_c):
        for point, oi in zip(fI_proj, currI):  
            ...
            if [oi,fi] in match_i:      
                ...
                fi_p_3D.append([iz[0], iz[1], point[0], xerr, point[1], yerr, oi, fi[0], len(fi), 0])
    ...
    for oz, fo in zip(of_z, curr_fO_c):
        for point, oo in zip(fO_proj, currO):  
            ...
            if [oo,fo] in match_o:
                ...
                fo_p_3D.append([oz[0], oz[1], point[0], xerr, point[1], yerr, oo, fo[0], len(fo), 1])
    ...
    if parameterisation == 'angle': 
        lines_fit, chisq, fit_status, cov_mats, cov_stats = track_selection_fitting_3D_4layers_root_angles(new_clean_combs) #TODO choose
    ...
    if len(chisq) != 0:
        ...
        paras_extra, pt_pairs = extrapolation_method(clean_combs_1)
        ...
        if len(paras_extra) > 1:
                ...
                dist_points, n_clust, m_cluster_arr, dists, cl_pts, largest_cl, covmat_largest_cl, total_covar_av, inds_largest_cl, av_dist_l_cl = cluster_finding_midpoints(paras_extra) #, chisq, clean_combs)
                unique_l_cl_inds = np.unique(inds_largest_cl).astype(int)
             
                print "unique indices of tracks in cluster", unique_l_cl_inds
                print " number of lines in cluster: ", len(unique_l_cl_inds)
                ...
                lines_fit_after_select_unique =  [paras_extra[i] for i in unique_l_cl_inds]  
                
                if eventplot3D == True:
                    for n, paramsl in enumerate(lines_fit_after_select_unique ):
                        x0,y0,z0 = line_param_extra(1000.0,paramsl)    # get a point on the current line (this is for plotting)
                        x1,y1,z1 = line_param_extra(-1000.0,paramsl)   # get another point
                ...
                """if vertexfit_choice == 'manual':
                    if parameterisation == 'angle':
                        ...
                        vertex_fitted, vertex_fitted_covar, hesse_bool = do_the_vertexfit_2(vertex_estimate, lines_fit_after_select_unique, cov_mats_1)      
    ...
    return len(paras_extra), mean_d_pt, av_d_pt, n_clust, largest_cl, dists, vertex_fitted, vertex_fitted_covar, covmat_largest_cl, total_covar_av, np.mean(av_dist_l_cl), eventplot3D, hesse_bool, nr_lines_fit_after_select_unique  

    
################################################################################################################################################
#########################################################################################################################################################################
def line_param_extra(t,p): # parameter form of a straight line 
    #x = p[0] + p[1]*t
    #y = p[2] + p[3]*t
    #z = t
    ####
    ...
    return x,y,z     
################################################################################################################################################
def extrapolation_method(track_combs_):  

    paras = [] 
    points_tr = []     
        
    for k_ in track_combs_:
        k_ = np.array(k_)
        k_ = k_[k_[:,7]!=999]
        ...
        if len(k_) == 2:
            ...
            """px = x1 - y1*(x1 - x2)/(y1 - y2) 
            pz = z1 - y2*(z1 - z2)/(y1 - y2)
            ...
            points_tr.append([x1 ,y1, z1, x2, y2, z2])
            paras.append([px, py, pz, dx, dy, dz])
        
        #for h in k_:
        #    print k_
        #    print " "            
                
    return paras , points_tr      
################################################################################################################################################
################################################################################################################################################    

        
def cluster_finding_midpoints(lines_fit_): #, chisq_, clean_combs_):
        ...
        total_covar_average = [[999,999,999],[999,999,999],[999,999,999]]
        covar_largest_cl = [[999,999,999],[999,999,999],[999,999,999]]
        ...
        for n, params in enumerate(lines_fit_):
            for m, params1 in enumerate(lines_fit_):
                ...
                x0,y0,z0 = line_param_extra(1.0,params)    # get a point on the current line
                x1,y1,z1 = line_param_extra(5.0,params)   # get another point on the line

                x01,y01,z01 = line_param_extra(2.0,params1)    # get a point on the current line 
                x11,y11,z11 = line_param_extra(4.0,params1)   # get another point on the line               
                ####################################################
        ...
        if len(mean_nps) > 0:
            ...
            return mean_nps, 0, [], 999, 999, [999], [[999,999,999],[999,999,999],[999,999,999]], [[999,999,999],[999,999,999],[999,999,999]], [], [999]
