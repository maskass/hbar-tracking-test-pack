...
def track_finding_4layers(polygon_hitlist, h, nO, nI, I, O, zposII, zposOO,
    ...
    if len(chisq) != 0:
        ...
        if len(clean_combs_1) > 1:
            ...
            if len(lines_fit_after_select) > 1:
                ...
                dist_points, n_clust, m_cluster_arr, dists, cl_pts, largest_cl, covmat_largest_cl, total_covar_av, inds_largest_cl, av_dist_l_cl = cluster_finding_midpoints_cluster2d(lines_fit_after_select, angle_cut, cov_mats_1) #, chisq, clean_combs)
    ...
    return len(lines_fit_after_select), mean_d_pt, av_d_pt, n_clust, largest_cl, dists, vertex_fitted, vertex_fitted_covar, covmat_largest_cl, total_covar_av, np.mean(av_dist_l_cl), eventplot3D, hesse_bool, nr_lines_fit_after_select_unique  
...
def cluster_finding_midpoints_cluster2d(lines_fit_, angle_cut_, cov_mats_): #, chisq_, clean_combs_):
        ...
        for k, col in zip(unique_labels, colors):
            ...
            mean_nps_curr =  mean_nps[class_member_mask & core_samples_mask]
            ...
            #print mean_nps_curr[:,0:3]
            
            
            # calculate weighted mean of point could on cluster
            #av_clus_xyz = np.average(mean_nps_curr[:,0:3], weights = 1.0/(curr_dist), axis = 0) #FIXME                       
            #exit(0)
            av_clus_xyz = np.mean(xyz2, axis = 0)            
            ################################################
            """print " ---------------------------------------------------", av_clus_xyz
